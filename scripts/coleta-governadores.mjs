...
function candidateName(header){
  ...
  s=norm(s);
  if(!s||/outros|indecis|branco|nulo|nenhum|vantagem|não sabe|nao sabe/i.test(s)) return null;
  return s;
}
...
async function discoverPages(){
  const searches=[
    "Pesquisas eleitorais para a eleição estadual de 2026",
    "Pesquisas de opinião eleição estadual 2026 governador"
  ];
  const titles=new Set();
  for(const srsearch of searches){
    const j=await api({action:"query",list:"search",srsearch,srlimit:"50",srnamespace:"0"});
    for(const item of j.query?.search||[]){
      if(/pesquis/i.test(item.title)&&/2026/.test(item.title)) titles.add(item.title);
    }
  }
  // Fallback: tenta títulos previsíveis para todas as UFs.
  for(const name of Object.values(STATES)){
    titles.add(`Pesquisas eleitorais para a eleição estadual de 2026 em ${name}`);
    titles.add(`Pesquisas de opinião para a eleição estadual de 2026 em ${name}`);
  }
  return [...titles];
}
...
export async function collect(){
  ...
  const titles=await discoverPages(), all=[], errors=[];
  for(const title of titles){
    const st=stateFromTitle(title); if(!st) continue;
    try{
      const page=await fetchPage(title);
      if(!page.html) continue;
      const url=`https://pt.wikipedia.org/wiki/${encodeURIComponent(page.title.replace(/ /g,"_"))}`;
      const parsed=parseStateHtml(page.html,{...st,title:page.title,url});
      if(parsed.length) all.push(...parsed);
    }catch(e){
      if(!/missingtitle|does not exist|não existe/i.test(e.message)) errors.push(`${title}: ${e.message}`);
    }
  }
  ...
  writeFileSync(join(DADOS,"_governadores_status.json"),JSON.stringify({
    ultima_execucao:new Date().toISOString(), paginas_tentadas:titles.length,
    pesquisas_extraidas:polls.length, estados_com_agregado:Object.keys(estados).length, erros:errors.slice(0,30)
  },null,2)+"\n");
